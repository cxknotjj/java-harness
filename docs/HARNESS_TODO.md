...
## 方向定调（2026-08-28 / 2026-09-06 评估收拢）

- **现状**：单用户场景完整跑通——两路径架构闭环（RouteJudge 分流 + StateGraph 编排）、沙箱容器级隔离 + 按专家分配工具权限（服务端硬边界）、进度全链路可视 + 断连优雅处理、CLI 对标 Claude Code 体验。
- **短板**（按风险排序）：可靠性（限流/熔断/降级仍缺，重试已有 `LlmRetry`）、能力面（web search 缺位，RAG/MCP 已完成）、编排智能化（无重规划/依赖声明）、部署与安全（无容器化交付、无接口鉴权）。
- **定调**：主线 = 能力扩展（web search → RAG ✓ → MCP ✓）；支线 = 可靠性收口（P0）；中期 = 编排智能化（P2）；远期 = 工程化可观测 + 产品化（P3）。
- **增补定档**（2026-09-06）：Goal 失败重投 → P2；长期记忆滚动摘要 → P2；多实例水平扩展、Agent 行为回归评测集 → P3；多模态输入 → 远期。
...
- [x] **执行层收敛（2026-09-11）**：路径 A/B 流式脚手架（重试/取消/预算记账/空帧跳过）双实现漂移消除——`AgentChatCaller` 抽出 `tokenStream` 共用管道核与 `callWithAssembly`/`streamWithAssembly` 直传入口，`GeneralAssistantAgent` 薄适配化（仅装配差异声明 + 响应式流终结钩子）；`GeneralAssistantAgentScaffoldTest` 基线测试锁定执行层行为。全量 446 用例通过（392e8dd）
...
- [x] **Token 预算统一控制**（2026-09-08 落地，详见 `docs/guides/context-optimization.md` 第五节）：预算集中配置（`app.context.*` / `ContextBudgetProperties`），消费侧新增两项——
  ...
  - 遗留已拆出为未完成条目（P1「聚合前子任务结果预算分摊裁剪」）
- [x] **工具调用与 MCP 日志记录**：`ToolCallTracer` 此前仅装饰 ToolCallback 发 `tool`/`tool-done` SSE 进度行（无 emitter 的调用链路不可见），工具调用不落库——`llm_call_log` 只记 LLM 调用，工具侧无成本/审计账本；MCP server 连接/发现失败仅 warn 单行，无 server 维度可查询记录
  ...
  - 注：验收中的 `/api/tool-calls` 查询端点不在本次范围，已拆出为未完成条目（P3）；本次验收以单测覆盖（411 通过）
- [x] **链路追踪（自建轻量方案）**：记录 LLM 调用耗时与 token 消耗——`llm_call_log` 表 + `LlmCallRecorder` 异步落库，`GET /api/llm-calls` 查询；Micrometer Tracing / OpenTelemetry 完整链路留待按需升级（见 P3「完整链路追踪」）
  - 验收 ✅：可经 `/api/llm-calls?sessionId=` 按会话查询每次调用的耗时/token/成败
...
- [x] 修复目前会话无法创建 goal 的问题
- [x] 检查 agent 角色提示词是否被写入了会话消息中（结论：未写入，角色段每请求组装不落库）
- [x] 增加会话 agent 切换功能：当在会话内切换 agent，会话表的 agentId 同步更改
- [x] 修复进入多 agent 时请求卡住的问题
